...
async def upload_data(request: Request,project_name: str, file: UploadFile,app_settings: Settings = Depends(get_settings)):
    ...
    file_path,file_name = DataController().generate_unique_file_path(orig_file_name=file.filename, project_name=project_name)
    try:
    ...
    asset_resource = Asset(
        asset_project_uuid=project.project_uuid,
    ...
    return JSONResponse(
            status_code=status.HTTP_200_OK,
             ...
             "asset_id": str(asset_record.asset_uuid),
            #  "project_name":str(project._id)
            }
             )
@data_router.post("/process/{project_name}")
async def process_endpoint(request: Request,project_name: str, process_request: ProcessRequest):
    ...
    asset_model=await AssetModel.create_instance(db_client=request.app.db_client)
    if do_reset == 1:
        await chunk_model.delete_chunks_by_project_uuid(project_uuid=project.project_uuid)
    project_file_ids={}
    ...
    if file_id:
        ...
        asset_record = await asset_model.get_asset_record(asset_name=file_id,project_name=project.project_uuid)
        if asset_record is None:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={"message": ResponseSignal.FILE_NOT_FOUND.value}
                )
        project_file_ids={asset_record.asset_uuid: asset_record.asset_name}
    else:
        assets = await asset_model.get_all_project_assets(project_uuid=project.project_uuid,asset_type=AssetTypeEnum.FILE.value)
        project_file_ids={
            record.asset_uuid: record.asset_name for record in assets
            }
    inserted_count=0
    files_cout=0
    ...
    for asset_id,file_id in project_file_ids.items():
        ...
        file_chunks_records=[
            DataChunk(
                datachunk_text=chunk.page_content,
                datachunk_metadata=chunk.metadata,  #type: ignore
                datachunk_order=idx+1,    
                datachunk_project_uuid=project.project_uuid,
                datachunk_asset_uuid=asset_id
            )
